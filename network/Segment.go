...
const (
  ...
  MAX_SEGMENT_PAYLOAD_SIZE = 688
  MAX_SEGMENT_SIZE = MAX_SEGMENT_PAYLOAD_SIZE + SEGMENT_HEADER_SIZE
)
...
func SegmentsFromBytes(isResponder bool, protocolId int, b []byte) []*Segment {
  ...
  for i := 0; i < len(b); i += MAX_SEGMENT_PAYLOAD_SIZE {
    res = append(res, SegmentFromBytes(isResponder, protocolId, b[i:common.Min(i+MAX_SEGMENT_PAYLOAD_SIZE, len(b))]))
  }
