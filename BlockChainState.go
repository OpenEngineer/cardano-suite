...
type BlockChainState struct {
  PrevBlock     Hash  // not relevant when first block id of main chain is genesis
...
func NewGenesisBlockChainState() *BlockChainState {
  return &BlockChainState{
    ...
    []Fork{
      Fork{
        -1, 0, []BlockHeader{NewBlockHeader(NewGenesisHash(), 0, 0, HashFromString(""))}, []NodeState{},
      },
    },
    &sync.RWMutex{},
  }
...
func (b *BlockChainState) findNodeFork(nodeId Hash) int {
...
func (b *BlockChainState) findBlock(fId int, headerHash Hash) (uint64, int) {
  for i, f := range b.Forks {
    if fId == -1 || fId == i {
...
// 
func (b *BlockChainState) RollForward(nodeId Hash, header *BlockHeader) error {
  b.mutex.Lock()
...
func (b *BlockChainState) RollBackward(nodeId Hash, point *Point) error {
  b.mutex.Lock()
...
func (b *BlockChainState) updateNodeState(fId int, nodeId Hash, bId uint64) {
  for i, f := range b.Forks {
    if i != fId {
...
func (b *BlockChainState) SetNodeIntersect(nodeId Hash, point *Point) error {
  bId, fId := b.findBlock(-1, point.HeaderHash)
