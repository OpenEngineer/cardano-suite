...
func (c *Connection) handleBlockFetchSegment(b []byte) error {
  msgs_, err := appendToCBORBuffer(c.blockFetchBuffer, b, MAX_PACKET_BUFFER_SIZE)
  if err != nil {
    return err
  }
    ...
    for {
      ...
      case *BlockFetchBlock:
        ...
        if c.BlockFetchEventEgress != nil {
          ...
          str:= ReflectUntyped(msg.Block.Block.Untyped)
...
func (c *Connection) FetchSingleBlock(slotId uint64, headerHash Hash) (*Block, error) {
  errCh := make(chan error)

  c.BlockFetchEventEgress = make(chan *Block)

  c.ListenNohup(errCh)

  point := &Point{false, slotId, headerHash}
